...
**Currency: EUR only.** All amounts, formatting, and copy use Euros (€) — never USD ($) or INR (₹).
...
Windows venv layout (`venv/Scripts/`, not `venv/bin/`).
...
Each Bash tool call starts a fresh shell, so `source venv/Scripts/activate` must be re-run (or chained with `&&`)
in any command that needs the venv.
